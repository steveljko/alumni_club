<nav class="w-full border-b border-gray-200 bg-white">
    <div class="container mx-auto w-full py-3">
        <div class="flex items-center justify-between">
            ...
            <div id="account"
                ...
                <div id="account_dropdown"
                    ...
                    tabindex="-1">
                    <div class="right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md border border-gray-200 bg-white shadow-lg outline-none md:absolute"
                        role="menu">
                        <div class="px-4 py-3">
                        ...
                        <div class="py-1">
                            ...
                            <a class="flex w-full cursor-pointer justify-between px-4 py-2 text-left text-sm leading-5 text-gray-700 hover:bg-gray-100"
                                role="menuitem"
                                href="{{ route('auth.settings') }}">Account settings</a>
                            <a class="flex w-full cursor-pointer justify-between px-4 py-2 text-left text-sm leading-5 text-gray-700 hover:bg-gray-100"
                                role="menuitem"
                                hx-delete="{{ route('auth.logout') }}"
                                hx-headers='{"X-CSRF-TOKEN": "{{ csrf_token() }}"}'>Sign out</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- mobile nav -->
        <div id="account_dropdown_mobile"
            class="hidden w-full border-t border-gray-200 transition duration-500 md:hidden">
            <div role="menu">
                <div>
                    <p class="text-sm leading-5">Signed in as</p>
                    <p class="truncate text-sm font-medium leading-5 text-gray-900">{{ auth()->user()->name }}</p>
